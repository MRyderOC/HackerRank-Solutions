...
DoublyLinkedListNode* sortedInsert(DoublyLinkedListNode* llist, int data) {
    ...
    while (tmp->data <= new_node->data) {
        // Check if the tmp is the last node in the DoublyLinkedList
        if (tmp->next == nullptr) {
            is_last_node = true;
            break;
        }
    ...
    // Do the operation if new_node should be appended at the end of DoublyLinkedList
    if (is_last_node) {
        tmp->next = new_node;
        new_node->prev = tmp;
        return llist;
    }
    // Check if tmp is the first node of DoublyLinkedList and do the related operation
    if (tmp == llist) {
        new_node->next = llist;
        llist->prev = new_node;
        return new_node;
    ...
    DoublyLinkedListNode *prev_node = tmp->prev;
