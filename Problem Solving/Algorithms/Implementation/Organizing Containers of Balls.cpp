...
string organizingContainers(vector<vector<int>> container) {
    vector<int> containers_cap, types_sum(container[0].size(), 0);

    for (auto cont: container) {
        containers_cap.push_back(accumulate(cont.begin(), cont.end(), 0));
        for (int i = 0; i < cont.size(); i++) {
            types_sum[i] += cont[i];
        }
    }
    sort(containers_cap.begin(), containers_cap.end());
    sort(types_sum.begin(), types_sum.end());

    if (containers_cap == types_sum)
        return "Possible";
    return "Impossible";
